      ...
      <div class="col-lg-12">
         ...
         <div class="card card-outline-info">
            <div class="card-header">
               <h4 class="m-b-0 text-white">Update Stock Details</h4>
            </div>
            <div class="card-body">
               <h4 class="card-title"></h4>
               <form method="POST" enctype="multipart/form-data"  onsubmit="document.getElementById('form').disabled=true;document.getElementById('form').innerHTML='Submitting, Please wait...';" action="<?= site_url('purchase_order/update_purchase_order_invoice'); ?>">
               ...
               <input type="hidden" name="ref_branch_id" value="<?php echo $purchase_order[0]->ref_branch_id; ?>">
                  <div class="row">
					  <div class="col-md-4">
					 ...
					 <table id="product_particulars_list" class="table color-bordered-table muted-bordered-table">
						<thead>
							<tr>
								...
								<th align="left" width="10%">Batch No</th>
								<th align="left" width="10%">Price</th>
								<th align="left" width="8%">Qty</th>
								<th align="left" width="10%">Manuf Date</th>
						...
						<tbody id="product_body">
							...
							?>
										<tr class="product_row_<?php echo $val->ref_product_id; ?>" id="row_<?= $val->ref_product_id?>">
											...
											<input type="hidden" id="balance_<?=$val->ref_product_id?>" value="<?= $val->qty - $po_product_qty[$val->ref_product_id] ?>">

										</td>
											<td align="left"><?= $val->qty - $po_product_qty[$val->ref_product_id] ?></td>
											<?php if($val->qty - $po_product_qty[$val->ref_product_id] > 0){ ?>
											<td align="left"><input type="text" class="form-control product_batch_<?= $val->ref_product_id?>" name="tbl_product_batch[<?= $val->ref_product_id; ?>][product_batch_name][]" value="">
												<input type="hidden" id="balance_to_receive_<?=$val->ref_product_id?>" value="0" name="tbl_product_batch[<?= $val->ref_product_id; ?>][product_balance_qty][]">
											</td>
											<td align="left"><input type="text" class="form-control product_batch_price_<?= $val->ref_product_id?>" id="<?= $val->ref_product_id?>" name="tbl_product_batch[<?= $val->ref_product_id; ?>][price][]" value="<?= $product_price?>"></td>
											<td align="left"><input type="text" class="form-control cur_qty product_batch_qty_<?= $val->ref_product_id?>" id="<?= $val->ref_product_id?>" name="tbl_product_batch[<?= $val->ref_product_id; ?>][quantity][]" value=""></td>
											<td align="left"><input type="text" class="form-control datepicker product_batch_<?= $val->ref_product_id?>" name="tbl_product_batch[<?= $val->ref_product_id; ?>][manufacture_date][]" value=""></td>
											<td align="left"><input type="text" class="form-control datepicker product_batch_<?= $val->ref_product_id?>" name="tbl_product_batch[<?= $val->ref_product_id; ?>][expiry_date][]" value=""></td>
											<td align="left"><a class="add_batch btn btn-success" id="<?= $val->ref_product_id; ?>">Add Batch</a></td>
										<?php }else{ ?>
											<td colspan="6"></td>
										<?php } ?>
										</tr>
                  ...
                  <div class="text-right">
                     <button class="btn btn-success" type="submit" id="form">Update</button>

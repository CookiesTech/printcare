...
class Purchase_Order extends CI_Controller
{
    function __construct()
    {
        ...
        if($this->uri->segment(2) == 'update_purchase_order_invoice'){
			$data['title'] = 'Invoice';
		}else{
     ...
     function edit()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
           ...
           redirect('purchase_order');           
			...
			$data['proforma_invoice']	= $this->Common_model->getDetails('purchase_order','purchase_order_id',$id);
			$data['proforma_invoice_particulars']	= $this->Common_model->getDetails('purchase_order_particulars','ref_purchase_order_id',$id);
			//~ echo '<pre>';
			//~ print_r($data['proforma_invoice']);
			//~ print_r($data['proforma_invoice_particulars']);
			//~ echo '</pre>';
			//~ exit;
    ...
    {        
        ...
        if (!empty($delete_id)) {
            foreach ($delete_id as $key => $id) {
                ...
                $this->Common_model->removeRecord('product_request_document', 'ref_purchase_order_id', $id);
            }
        }
        if($res){
			$this->Common_model->TxnCommit();
   ...
   function send_purchase_order()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if (isset($res_sample) && !empty($res_sample)){
                $data['subject'] = 'Product Request - #' . $res_sample[0]->purchase_order_code . '';
	...
	function update_purchase_order_invoice(){
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            //~ echo '</pre>';
            //~ exit;
			$res = $this->Common_model->addRecord('invoice',$data,$id);			
			if(isset($po_particulars) && !empty($po_particulars)){
           ...
           redirect('purchase_order'); 
			...
			//~ print_r($data['purchase_order_particulars']);
			//~ echo '</pre>';
			//~ exit;
	...
	function generate_invoice_pdf($id,$data=''){
		...
		$f_name = 'uploads/invoice/'.str_replace('/','_',$data['invoice'][0]->invoice_no).'.pdf';
		$file_name = FCPATH.$f_name;
		$this->Common_model->exportPdf($header_html,$footer_html,$html,$file_name,'P','F');

					 ...
					 <table id="product_particulars_list" class="table color-bordered-table muted-bordered-table">
						<thead>
							<tr>
								...
								<th align="left" width="15%">Patch No</th>
						...
						<tbody id="product_body">
							<?php 
								$i = 1;
								if(isset($product_patch) && !empty($product_patch)){
									foreach($product_patch as $key => $val){ 	
							...
							?>
										<tr class="product_row_<?php echo $val->product_patch_id; ?>" id="row_<?= $val->product_patch_id?>">
											<td align="left"><?php echo getDateTimeFormat($val->added_date); ?></td>
											<td align="left"><?php echo $val->product_name; ?>
											<input type="hidden" name="tbl_product_patch[<?= $val->product_patch_id; ?>][ref_product_id]" value="<?php echo $val->ref_product_id; ?>" >
											</td>
											<td align="left"><?= $sku?></td>
											<td align="right"><?= $product_price?></td>
											<td align="left"><input type="text" class="form-control product_patch_<?= $val->product_patch_id?>" name="tbl_product_patch[<?= $val->product_patch_id; ?>][product_patch_name]" value="<?= $val->product_patch_name?>"></td>
											<td align="left">
												<input type="hidden" class="" id="" name="tbl_product_patch[<?= $val->product_patch_id; ?>][prev_quantity]" value="<?= $val->quantity?>">

												<input type="text" class="form-control cur_qty product_patch_qty_<?= $val->product_patch_id?>" id="<?= $val->product_patch_id?>" name="tbl_product_patch[<?= $val->product_patch_id; ?>][quantity]" value="<?= $val->quantity?>"></td>
											<td align="left"><input type="text" class="form-control datepicker product_patch_<?= $val->product_patch_id?>" name="tbl_product_patch[<?= $val->product_patch_id; ?>][manufacture_date]" value="<?= getDateFormat($val->manufacture_date)?>"></td>
											<td align="left"><input type="text" class="form-control datepicker product_patch_<?= $val->product_patch_id?>" name="tbl_product_patch[<?= $val->product_patch_id; ?>][expiry_date]" value="<?= getDateFormat($val->expiry_date)?>"></td>
											<td align="left"><a class="remove_product_patch btn btn-danger">Remove</a></td>
										</tr>

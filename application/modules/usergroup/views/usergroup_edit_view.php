	...
	<div class="row">
		<div class="col-lg-12">
			<div class="card card-outline-info">
				...
				<div class="card-body">
					<?php echo form_open('usergroup/edit/'.$usergroup['id']); ?>
							...
							<div class="form-group col-md-3 ">
								<div class="form-line">
									<input class="form-control" placeholder="User Group" type="text" name="name" value="<?php echo $usergroup['name']; ?>" required>
								</div>
							</div>
							...
							<table class="table table-striped table-borderless">
								...
								if(isset($pages)){
									foreach($pages as $page){ ?>
										<tr>
										<td><input type="checkbox" class="setrowpermission" id="row_<?php echo $i; ?>"><label for="row_<?php echo $i; ?>"><?php echo ucfirst($page['pages_name']); ?></label>  <i class="fa fa-question-circle" aria-hidden="true" title="<?php echo $page['pages_name']; ?>"></i></td>
										<?php 
										if(isset($actions)){
											foreach($actions as $action){ 
												if(in_array($page['pages_name'].'_'.$action['action'],$permission_allowed)){ ?>
													<td><input type='checkbox' id="<?php echo $action['action'].'_'.$i; ?>" class="<?php echo $action['action'].'_permission'; ?> row_<?php echo $i; ?>" name="permission[<?php echo $page['pages_name']; ?>][<?php echo $action['action']; ?>]" value='1' checked> <label for="<?php echo $action['action'].'_'.$i; ?>"><?php echo $action['action']; ?></label> </td>
												<?php }else{
												?>
												<td><input type='checkbox' class="<?php echo $action['action'].'_permission'; ?> row_<?php echo $i; ?>" id="<?php echo $action['action'].'_'.$i; ?>" name="permission[<?php echo $page['pages_name']; ?>][<?php echo $action['action']; ?>]" value='1'> <label for="<?php echo $action['action'].'_'.$i; ?>"><?php echo $action['action']; ?></label> </td>
											<?php } }
							   ...
							   <tr>
								<th colspan="8" align="center">Master Page Permission</th>
							   </tr>
									...
									foreach($master_pages as $page){ ?>
										...
										if(isset($actions)){
											foreach($actions as $action){ 
												...
												<?php }else{
												?>
												<td><input type='checkbox' id="<?php echo $action['action'].'_'.$i; ?>" class="<?php echo $action['action'].'_permission'; ?> row_<?php echo $i; ?>" name="permission[<?php echo $page['pages_name']; ?>][<?php echo $action['action']; ?>]" value='1'> <label for="<?php echo $action['action'].'_'.$i; ?>"><?php echo $action['action']; ?></label> </td>
											<?php } }
